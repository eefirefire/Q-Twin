...
const doc = new Document({
  sections: [{
    ...
    children: [
      ...
      h2("(c) Validation"),
      p("Correlation between early_displacement_30s and the corrected delta_f_probe is 0.9997 across the 44 valid chips — this is essentially an early readout of the same signal, which is exactly why it works as a feature (unlike the slope version, which was a genuinely different quantity). Threshold classification against the CURRENT -0.5 Hz SUCCESS/FAILURE rule (predict SUCCESS if displacement <= -0.5 Hz, matching FAILURE_THRESHOLD_HZ exactly, not the old 0 Hz cutoff) scores 97.7% (43/44 scoreable chips) at the 30s window, and holds flat at 97.7% across 15s/30s/45s/60s windows too."),
      p("Real range of early_displacement_30s across all 44 valid chips: -374.63 Hz to +107.93 Hz (20Mar_No.5 = -374.63 at the low end, 15Mar_No.21 = +107.93 at the high end). That width is the point -- a sub-1-Hz range couldn't support 97%+ classification accuracy; the threshold rule only works because real chips spread out this widely. (An earlier, much narrower -0.6 to +0.9 Hz figure was checked and traced to eyeballing only the two chips closest to zero -- 21Mar_No.8 at -0.04 and 21Mar_No.7 at +0.64 -- instead of the full 44-chip column; corrected here.)"),
      note("Update: an earlier draft of this section described the test as \"threshold-at-0\" (predict SUCCESS if displacement < 0) and reported 97.5% (39/40). Eva flagged that description as possibly stale against the current -0.5 Hz rule (fair catch -- the wording WAS wrong) and sent a re-verification showing a window-varying pattern (95.5%/93.2%/93.2%/95.5% across 15/30/45/60s). Independently re-derived from raw data before changing anything: the ORIGINAL 97.5%/39-40 number turns out to already have been computed against -0.5 Hz, not 0 Hz -- recomputing at 0 Hz on the same chip subset gives a different, window-varying result instead, so only the prose description was mislabeled, not the number. Using the fuller, more correct baseline (matching ingest_raw_curves.py exactly, including the No.7/8/9/10 no-CHI-file fallback, n=44 instead of 40) gives 97.7% (43/44), also flat across all four windows -- this is the number now used above. Could not reproduce Eva's window-varying pattern under either baseline; traced the specific flip chip (21Mar_No.9) through all four windows and found the -0.5 Hz correction only ever improves or holds its classification, never trades one window's gain for another's loss. Flagged back to Eva as a specific, reproducible disagreement to double-check (her \"new/-0.5Hz\" column matches this project's 0 Hz numbers almost exactly, which looks like a possible column swap on her end) rather than silently adopting numbers that don't independently reproduce. Full trace: clarifying_questions.md item 8, biomarker_window_reconciliation.py."),
