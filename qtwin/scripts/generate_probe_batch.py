...
def build_rows(kind: str, n: int, rng: np.random.Generator, concs: np.ndarray, forced_divergent: int = 0,
               sequences: list = None):
    """sequences, if given a list, gets one resampled-curve array appended per
    row (average of the two replicates' resampled curves) -- for Week 3
    Task 3's LSTM. Appending here (not recomputing later) guarantees the
    saved sequence is generated by the exact same RNG draw as the row's own
    CSV columns, not a second, potentially-diverged regeneration."""
    rows = []
    for i in range(n):
...
def main():
    ...
    # Week 3 Task 3: full resampled sequences, row-aligned with df above
    # (same order, same RNG draw) -- synthetic_batch_v1.csv / the CSVs never
    # stored raw curves, only summary stats, so this is the actual sequence
    # data the LSTM needs.
    seq_array = np.array(sequences)
    seq_path = OUT_DIR / "probe_synthetic_sequences.npz"
